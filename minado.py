...
async def tratar_botoes_minado(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if dados.startswith("min_pos_"):
        ...
        if partida["modo"] == "pvp":
            ...
            await mostrar_tabuleiro(query, partida, "✅ Continue ou passe a vez!")
        return
...
async def explodir(query, partida, uid):
    ...
    vencedor_nome = partida["p2_nome"] if partida["modo"]=="pvp" and uid==partida["p1_id"] else \
                     partida["p1_nome"] if partida["modo"]=="pvp" else "🤖 Máquina"
    if partida["modo"]=="ia" and uid==partida["p1_id"]:
