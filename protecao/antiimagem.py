...
async def executar_antiimagem(update, context, chat, user, message, get_db, is_admin, obter_punicao, salvar_punicao, apagar_aviso_futuro):
    ...
    try:
        ...
        if not cfg.get("antiimagem", True):
            return False
    except Exception:
        return False
    ...
    # ✅ 3. DETECTA IMAGEM (foto ou documento de imagem)
    ...
    if tipo_acao == "remover":
        try:
            ...
            aviso = await chat.send_message(
                f"🚨 {user.mention_html()} foi BANIDO por enviar fotos/imagens!",
                parse_mode="HTML"
            )
        ...
        try:
            ...
            aviso = await chat.send_message(
                f"🔇 {user.mention_html()} SILENCIADO por {tempo_mute} min por enviar fotos!",
                parse_mode="HTML"
            )
            asyncio.create_task(apagar_aviso_futuro(context, aviso))
        except Exception:
        ...
        if avisos >= 2:
            try:
                await context.bot.ban_chat_member(chat.id, user.id)
                aviso = await chat.send_message(
                    f"🚨 {user.mention_html()} BANIDO por insistir em enviar fotos!",
                    parse_mode="HTML"
                )
            ...
            aviso = await chat.send_message(
                f"⚠️ {user.mention_html()} PROIBIDO enviar fotos aqui! ({avisos}/2)",
                parse_mode="HTML"
            )
            asyncio.create_task(apagar_aviso_futuro(context, aviso))
    return True
