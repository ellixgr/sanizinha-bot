...
async def executar_antienquete(update, context, chat, user, message, get_db, is_admin, obter_punicao, salvar_punicao, apagar_aviso_futuro):
    ...
    try:
        ...
        if not cfg.get("antienquete", True):
            return False
    except Exception:
        return False
    ...
    if tipo_acao == "remover":
        try:
            ...
            aviso = await chat.send_message(
                f"🚨 {user.mention_html()} foi BANIDO por {motivo_txt}!",
                parse_mode="HTML"
            )
        ...
        try:
            ...
            aviso = await chat.send_message(
                f"🔇 {user.mention_html()} SILENCIADO por {tempo_mute} min por {motivo_txt}!",
                parse_mode="HTML"
            )
            asyncio.create_task(apagar_aviso_futuro(context, aviso))
        except Exception:
        ...
        if avisos >= 2:
            try:
                await context.bot.ban_chat_member(chat.id, user.id)
                aviso = await chat.send_message(
                    f"🚨 {user.mention_html()} BANIDO por insistir em {motivo_txt}!",
                    parse_mode="HTML"
                )
            ...
            aviso = await chat.send_message(
                f"⚠️ {user.mention_html()}, o envio de enquetes não é permitido neste grupo! ({avisos}/2)",
                parse_mode="HTML"
            )
            asyncio.create_task(apagar_aviso_futuro(context, aviso))
    return True
