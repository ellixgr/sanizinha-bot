    ...
    if tipo_acao == "remover":
        try:
            ...
            aviso = await chat.send_message(
                f"🚨 {user.mention_html()} foi BANIDO por enviar figurinhas!",
                parse_mode="HTML"
            )
        ...
        try:
            ...
            aviso = await chat.send_message(
                f"🔇 {user.mention_html()} SILENCIADO por {tempo_mute} min por enviar figurinhas!",
                parse_mode="HTML"
            )
            asyncio.create_task(apagar_aviso_futuro(context, aviso))
        except Exception:
        ...
        if avisos >= 2:
            try:
                await context.bot.ban_chat_member(chat.id, user.id)
                aviso = await chat.send_message(
                    f"🚨 {user.mention_html()} BANIDO por insistir em enviar figurinhas!",
                    parse_mode="HTML"
                )
            ...
            aviso = await chat.send_message(
                f"⚠️ {user.mention_html()} PROIBIDO enviar figurinhas aqui! ({avisos}/2)",
                parse_mode="HTML"
            )
            asyncio.create_task(apagar_aviso_futuro(context, aviso))
    return True
